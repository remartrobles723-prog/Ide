...
enum class IntegerOption(override val propertyName: String, stage: ApiStage) : Option<Int> {
   ...
   * number of snapshots. 0: disable Android Test Retention. <0: enable Android Test Retention with
  ...
  /**
   * The timeout duration in minute for Gradle Managed Device setup steps (booting AVD and creating
   * snapshot image) If the value is 0 or negative value, it will never time out.
   */
  ...
  /**
   * The timeout duration in seconds for install apk If the value is 0 or negative value, it will be
   * set to a default value by UTP.
   */
