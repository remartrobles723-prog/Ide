...
sealed class ApiStage(status: Option.Status) : Stage(status) {
   ...
   *
   * It may become stable or may be removed in a future release (see stage [Stable] and [Removed]).
   */
  ...
  /**
   * Indicates that the API will be removed soon because it was not / is no longer useful (see stage
   * [Removed]).
   *
...
sealed class FeatureStage(status: Option.Status) : Stage(status) {
   ...
   *
   * It may or may not be enabled by default. If it is not yet enabled by default, it will likely be
   * enabled by default in a future release.
   *
   * Eventually, the feature will likely be enforced (see stage [SoftlyEnforced] and [Enforced]). In
   * some cases, it may be removed (see stage [Deprecated] and [Removed]).
   *
   * DISCOURAGED USAGE: The use of this stage is actually discouraged as it doesn't specify a clear
   * timeline and features may stay in this stage for too long, thus increasing maintenance cost to
   * AGP and users. Consider using [SoftlyEnforced] or [Deprecated] instead.
   */
   ...
   * been removed.
   *
   * @param enforcedVersion the version when the feature is enforced and the corresponding [Option]
   *   was removed
   * @param additionalMessage the additional message to be shown if the [Option] is used
   */
   ...
   * has been removed.
   *
   * @param removedVersion the version when the feature and the corresponding [Option] were removed
   * @param additionalMessage the additional message to be shown if the [Option] is used
   */
  class Removed(removedVersion: Version, additionalMessage: String? = null) :
