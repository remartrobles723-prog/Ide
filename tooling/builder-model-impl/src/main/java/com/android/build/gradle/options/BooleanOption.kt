...
enum class BooleanOption(
   ...
   * during test will be uninstalled after test finishes. Setting it to true means that the AGP will
   * leave the test APKs untouched after test.
   *
   * Default is false
   */
   ...
   *
   * This option is useful when some dependencies are missing from the input artifact's dependencies
   * (see bug 230454566), so the full classpath is needed.
   *
  ...
  /**
   * When enabled, "-show-kernel" and "-verbose" flags are used when running an Android emulator for
   ...
   *
   * When this flag is enabled, it allows a user to use any old API level devices regardless of its
   * instability.
   */
  ...
  /**
   * When enabled, Gradle Managed Device allows a custom managed device type that can be provided by
   * a plugin by implementing ManagedDeviceTestRunner APIs.
   */
  ...
  USE_NEW_JAR_CREATOR("android.useNewJarCreator", true, FeatureStage.Enforced(Version.VERSION_8_0)),
  USE_NEW_APK_CREATOR("android.useNewApkCreator", true, FeatureStage.Enforced(Version.VERSION_8_0)),
